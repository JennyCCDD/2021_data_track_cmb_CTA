...
from predict import predict  # 导入预测方法

MAX_DAY = 30
BEGIN_EVAL = 900
DAY_MAX_TSID = 9000

tick_test_file = "tick_test-fake.csv"            # fake 文件只是为了展示eval的形式，fake的受益与评判无关
label_test_file = "label_test-fake.csv"
...
def main(_):
    ...
    for day in range(MAX_DAY):
        ...
        for ts in range(DAY_MAX_TSID):
            ...
            # else:
            result={1,2}
            pre_act = choice(result)
#             pre_act = predict(data_test)
            if ts > BEGIN_EVAL:
